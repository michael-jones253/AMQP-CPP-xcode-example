...
Threading/performance considerations. The receiver app has command line options to either process the incoming messages in the context of the callback from the Copernica library or to have it queued and processed by another thread (via the MyTaskProcessor class). I can see from the debugger that the callback from the Copernica library is being invoked in the context of the network read thread. So the MyTaskProcessor avoids holding up network reads when processing the messages, thus reducing the chances of flow control holding up the network writing in the RabbitMQ server. Similarly the debugger tells me that sending of the message Ack involves a network write in that same thread context so sending of acks are also handled by another thread (via the MyAckProcessor class). Benchmarking on the loopback does not show a performance improvement with the threaded option, however the loopback does not behave the same as the LAN/WAN. In the situation where a bursts of messages are arriving and being allowed to pile up on the task  and ack queues and message processing is blocking e.g. remote database operations, I would expect this state of affairs to outperform holding up network operations from the server and holding up processing on network writes.
